...
namespace JPBotelho
{
    ...
    */
    public class CatmullRom
    {
        ...
        public CatmullRom(Vector3[] controlPoints, Vector3[] _normalPoints, int resolution, bool closedLoop)
        {
            if(controlPoints == null || controlPoints.Length <= 2 || resolution < 2)
            {
                throw new ArgumentException("Catmull Rom Error: Too few control points or resolution too small");
            ...
            this.controlPoints = new Vector3[controlPoints.Length];
            this.normalVectors = new Vector3[controlPoints.Length];
            
            for(int i = 0; i < controlPoints.Length; i++)
            {
                this.controlPoints[i] = controlPoints[i];             
                this.normalVectors[i] = _normalPoints[i];             
            }
        ...
        public void Update(Vector3[] controlPoints, Vector3[] normals)
        {
            if(controlPoints.Length <= 0 || controlPoints == null)
            {
                throw new ArgumentException("Invalid control points");
            ...
            this.controlPoints = new Vector3[controlPoints.Length];
            this.normalVectors = new Vector3[normals.Length];
            for(int i = 0; i < controlPoints.Length; i++)
            {
                this.controlPoints[i] = controlPoints[i];             
                this.normalVectors[i] = normals[i];             
            }
        ...
        private void GenerateSplinePoints()
        {
            ...
            for (int currentPoint = 0; currentPoint < controlPoints.Length - closedAdjustment; currentPoint++)
            {
                ...
                else
                {
                    ...
                    n1 = normalVectors[currentPoint + 1];
                }
                ...
                for (int tesselatedPoint = 0; tesselatedPoint < resolution; tesselatedPoint++)
                {
                    ...
                    CatmullRomPoint point = Evaluate(p0, p1, m0, m1,n0,n1, t);
        ...
        public static CatmullRomPoint Evaluate(Vector3 start, Vector3 end, Vector3 tanPoint1, Vector3 tanPoint2, Vector3 normal1,Vector3 normal2, float t)
        {
            Vector3 position = CalculatePosition(start, end, tanPoint1, tanPoint2, t);
            Vector3 tangent = CalculateTangent(start, end, tanPoint1, tanPoint2, t);
            Vector3 normal = CalculateNormals(normal1, normal2, t);

            return new CatmullRomPoint(position, tangent, normal);
        }

        private static Vector3 CalculateNormals(Vector3 _normal1, Vector3 _normal2, float _f)
        {
            return _normal1;
        }
